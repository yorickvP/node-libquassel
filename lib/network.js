...
Network.prototype.setIrcUsersAndChannels = function(uac) {
    ...
    if (uac.Users) {
        const users = uac.Users.nick.map(nick => new IRCUser(nick))
        for (const key in uac.Users) {
            uac.Users[key].forEach((val, i) => users[i][key] = val)
        }
        users.forEach(user => {
            user.id = `${user.nick}!${user.user}@${user.host}`
            this.users.set(user.nick, user)
        })
    } else if (uac.users) {
    ...
    if (uac.Channels) {
        ...
        uac.Channels.name.forEach((key, i) => {
            const channel = this.getBuffer(key);
            //Then attach users to channels
            for (const nick in uac.Channels.UserModes[i]) {
                let user = this.getUserByNick(nick);
                if (user !== null) {
                    channel.addUser(user, uac.Channels.UserModes[i][nick]);
                } else {
                    logger("User " + nick + " have not been found on server.");
                }
            }
        });
    } else if (uac.channels) {
        // Create Channels and attach them to network
