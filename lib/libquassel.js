...
Quassel.prototype.heartBeat = function(reply) {
    ...
    this.qtsocket.sendHeartbeat(new Date(), reply);
};
...
Quassel.prototype.handleStruct = function(obj) {
    ...
    switch (obj[0]) {
        case RequestType.Sync: {
            ...
            const objectName = obj[2] !== null ? obj[2].toString() : null
            ...
            switch(className) {
                ...
                case "IgnoreListManager":
                    ...
                    break;
                case "Identity":
                    identityId = parseInt(objectName, 10);
                    ...
                    if (identity) {
                        ...
                        self.log('Unknown Identity ' + objectName);
                    }
                    break;
                case "AliasManager":
                    switch(functionName) {
        ...
        case RequestType.InitData: {
            console.error("initdata handled in protocol")
            } break;
...
Quassel.prototype.handleInitData = function(className, objectName, params) {
    var networkId, bufferId, buffer, bufferName, network, bufferNetworkId,
        messageId, tmp, user, data, i, ind, bufferViewId;

    switch(className) {
        case "Network":
            network = this.handleInitDataNetwork(objectName, params);
            var syncRequest = [
            ...
            this.qtsocket.write(syncRequest);
            this.emit("network.init", network.networkId);
            break;
        case "BufferSyncer":
            var markerLinesData = params["MarkerLines"];
            var lastSeenData = params["LastSeenMsg"];
            if (lastSeenData !== null) {
                for (i=0; i<lastSeenData.length; i+=2) {
                    bufferId = lastSeenData[i];
                    messageId = lastSeenData[i+1];
                    buffer = this.networks.findBuffer(bufferId);
                    if (buffer !== null) {
                        this.emit('buffer.lastseen', bufferId, messageId);
                    } else {
                        this.log("Buffer #" + bufferId + " does not exists");
                    }
                }
            } else {
                this.log("Received null LastSeenMsg");
            }
            if (markerLinesData !== null) {
                ...
                this.log("Received null markerLines");
            }
            break;
        case "IrcUser":
            tmp = splitOnce(objectName, "/");
            ...
            user = this.networks.get(networkId).getUserByNick(tmp[1]);
            if (user !== null) {
                user.devour(data);
                this.emit('network.adduser', networkId, tmp[1]);
            }
            break;
        case "IrcChannel":
            tmp = splitOnce(objectName, "/");
            ...
            bufferName = tmp[1];
            buffer = this.networks.get(bufferNetworkId).getBufferCollection().getBuffer(bufferName);
            buffer.topic = data.topic;
            buffer.setActive(true);
            this.emit('channel.topic', bufferNetworkId, bufferName, data.topic);
            this.emit('buffer.activate', buffer.id);
            break;
        case "BufferViewManager":
            data = params["BufferViewIds"];
            for (ind in data) {
                this.sendInitRequest("BufferViewConfig", ""+data[ind]);
            }
            this.emit('bufferview.ids', data);
            break;
        case "BufferViewConfig":
            bufferViewId = parseInt(objectName, 10);
            data = params;
            this.bufferViews.set(bufferViewId, new BufferView(bufferViewId, data));
            for (ind in data.TemporarilyRemovedBuffers) {
                this.emit('bufferview.bufferhidden', bufferViewId, data.TemporarilyRemovedBuffers[ind], "temp");
            }
            for (ind in data.RemovedBuffers) {
                this.emit('bufferview.bufferhidden', bufferViewId, data.RemovedBuffers[ind], "perm");
            }
            this.emit('bufferview.orderchanged', bufferViewId);
            this.emit('bufferview.init', bufferViewId);
            break;
        case "IgnoreListManager":
            data = params;
        ...
        case "CoreInfo":
            ...
            break;
        default:
            this.log('Unhandled InitData ' + className);
    }
};
 ...
 * @protected
 */
Quassel.prototype.handleInitDataNetwork = function(objectName, params) {
    var networkId = parseInt(objectName, 10);
    var network = this.networks.get(networkId);
    network.devour(params);
    return network;
};
...
Quassel.prototype.sendInitRequest = function(classname, objectname) {
    this.qtsocket.sendInitRequest(classname, objectname);
};
...
Quassel.prototype.init = function() {
    ...
    this.client.once('data', function(data) {
        ...
        if (data.length != 4) {
            console.log("wrong amount of data in prehandshake", data)
        }
        var proto = ret & 0xff;
        var protoFeatures = (ret >> 8) & 0xffff;
        var connectionFeatures = ret >> 24;
        if (connectionFeatures & 0x01) {
            self.useSSL = true;
            self.log('Using SSL');
        }
        
        if (connectionFeatures & 0x02) {
            self.useCompression = true;
            self.log('Using compression');
        }
        
        self.qtsocket = new (protocol.getById(proto))(self.client, self.useCompression, self.useSSL, protoFeatures);

        // bind events on qtsocket
        self.qtsocket.on('struct', function(data) {
            self.handleStruct(data);
        })
        self.qtsocket.on('msgtype', function(data) {
            self.handleMsgType(data);
        })
        self.qtsocket.on('initdata', self.handleInitData.bind(self))
        .on('close', function() {
...
Quassel.prototype.connect = function() {
    ...
    this.client.connect(this.port, this.server, function(){
        ...
        protocol.protocols.forEach(function(protocol, i, protos) {
            let reply = protocol.id;
            reply |= protocol.features;
            if (i == protos.length - 1) reply |= 0x80000000;
            writer.writeUInt(reply);
        });
